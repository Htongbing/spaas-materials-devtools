...
const emptyValidate = value => {
  if (!value.trim()) return '该项不能为空，请重新输入';
  return true;
};

module.exports = async function init(cwd) {
  // 初始化项目类型
  const { initType, repository, commitMessage, branch } = await inquirer.prompt([
    {
      type: 'list',
      message: '请选择初始化类型',
      name: 'initType',
    ...
    },
    {
      ...
      name: 'associated',
      default: true,
      when: ({initType}) => initType === 'project'
    },
    {
      type: 'input',
      message: '请输入git仓库地址',
  ...
  // 选择初始化spaas项目
  if (initType === 'project') {
    const projectName = await initProject(cwd);
    // 关联仓库地址不为空，则执行下列命令
    if (repository) {
      const shellCommand = [
        `cd ${projectName}`,
      ...
      // 不为master分支则添加切换新分支命令
      if (branch !== 'master') shellCommand.splice(2, 0, `git checkout -b ${branch}`)

      // 执行命令
      shelljs.exec(shellCommand.join(' && '));
    };
    return;
  };
